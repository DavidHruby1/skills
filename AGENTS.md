...
- `@bash-agent`: Always use when several shell commands are expected to produce large outputs. Provide the exact commands, working directory, and dependency order. Require a compact report with each command's exit status and the evidence needed to assess its result.
