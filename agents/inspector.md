...
Accept `Axis: Standards | Spec | All` and `Mode: standalone | task`. Defaults are `All` and `standalone`. The code-review skill supplies one axis per independent invocation. Do not delegate. Use `anti-over-engineering` when assessing whether scope or complexity is justified, and `software-philosophy` when reviewing design decisions about module responsibilities, interfaces, dependency boundaries, data guarantees, or architectural trade-offs, not local syntax, naming, formatting, or helper choices that leave those decisions unchanged. Apply only guidance relevant to the assigned axis as review criteria, never as instructions to edit, execute tests, expand review scope, or override test-source exclusions. Do not load either for routine mechanical changes.
