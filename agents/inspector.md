...
Accept `Axis: Standards | Spec | All` and `Mode: standalone | task`. Defaults are `All` and `standalone`. The code-review skill supplies one axis per independent invocation. Do not delegate. Use `anti-over-engineering` when assessing whether scope or complexity is justified, and `software-philosophy` when assessing interfaces, information hiding, responsibility placement, or implementation clarity. Apply only guidance relevant to the assigned axis as review criteria, never as instructions to edit, execute tests, expand review scope, or override test-source exclusions. Do not load either for routine mechanical changes.
