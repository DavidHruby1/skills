...
Invoke `test-contract-auditor` at most twice per command invocation. Run it once after the complete substantive draft. Supply the exact user request and scope, active task path, selected basis, full current `GHERKIN.md`, relevant acceptance criteria and plan sections, applicable plan-wide constraints, and only the research, source, test, documentation, or history evidence needed to verify consequential claims. Supply a governing artifact in full only when narrower evidence cannot establish the contract safely.

Apply only blocking findings supported by governing evidence. Run the auditor a second and final time only when those corrections materially changed behavior, scope, level, Gherkin, or traceability. Do not re-audit wording, formatting, IDs, dates, or other mechanical metadata corrections. Never invoke a third audit in the same command invocation. When either audit returns `READY`, replace `Audit: Pending` with `Audit: Passed YYYY-MM-DD`. If the final allowed audit returns `REWORK`, apply only unambiguously mechanical corrections, leave `Audit: Pending`, and report the remaining blocking findings to the user instead of continuing an audit loop. The auditor never edits files, and its report is for correction rather than a separate artifact.
