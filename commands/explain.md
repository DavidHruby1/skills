...
3. **How it works**: source-backed code excerpts in execution order, with normal and failure traces placed beside the code they explain. Teach each excerpt in enough detail that a reader who knows the language syntax but not this codebase can reconstruct what happens. Explain it line by line or by small logical groups: what each identifier refers to, which values enter, which expressions are evaluated, which branches can run, what is returned or mutated, and why that part exists. Do not replace this walkthrough with a high-level paragraph.
...
5. **Connections and choices**: callers, dependencies, state, side effects, focused tests, design trade-offs, and genuinely unknown rationale. Include a clearly titled **Stav a jeho změny** section whenever the requested code reads or writes state. Inventory every relevant local state variable, reactive reference, store field, cache, mutable object, and persisted value. For each one, state where it is declared, its exact type and initial value when knowable, who reads it, who writes it, what event causes each change, and what behavior depends on it. Distinguish component-local, shared, derived, transient, and persisted state instead of grouping them under a generic “state” label.
...
Use exact, syntax-highlighted `<pre><code>` excerpts with visible line numbers and `path:start-end` labels. A `<pre><code>` block may contain only escaped source text: never place prose, lists, diagrams, callouts, navigation, or other page markup inside it, and close both tags immediately after the excerpt. Prefer 8-30 lines that capture one contract, transformation, branch, or side effect. Immediately after every excerpt, provide a detailed walkthrough in source order. Name the exact lines or small line ranges being discussed and explain their inputs, operation, output or effect, importance, and reason for being present. Expand non-obvious expressions, callbacks, destructuring, guards, framework helpers, and mutations; define important identifiers at first use. Explicitly connect each read and write to the state inventory and show representative before/after values when state changes. Do not merely paraphrase the code, repeat its syntax in prose, or say that a line “sets” or “processes” something without explaining the resulting value and its later use. Cover every important behavior, but summarize truly mechanical declarations. HTML-escape source text and redact credentials or personal data.

For reactive UI code, explain the framework's state model before relying on its vocabulary. For example, when Vue code uses `const bookingsBaseline = ref(...)`, explain that `bookingsBaseline` is a stable reactive wrapper object, that the wrapped current value lives in `bookingsBaseline.value` in script code, what its initial value represents, and which consumers Vue updates after assignment. For every `.value`, state access, setter, reducer, signal update, or equivalent operation, show the old value, the new value, the trigger, and why downstream code needs that change. Explain derived values (`computed`, selectors, memos), watchers/effects, lifecycle timing, and cleanup where present, including what would become stale or incorrect without them. Adapt this treatment to the actual framework; never assume that a reader already understands its reactivity conventions.
