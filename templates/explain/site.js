document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".function-canvas-shell").forEach((shell) => {
    const container = shell.querySelector(".function-graph");
    const source = shell.querySelector('script.function-graph-data[type="application/json"]');
    ...
    const details = shell.querySelector(".function-details");
    ...
    shell.querySelector("[data-graph-action='zoom-in']")?.addEventListener("click", () => {
      cy.zoom({ level: Math.min(cy.zoom() * 1.2, cy.maxZoom()), renderedPosition: center(container) });
    });
    shell.querySelector("[data-graph-action='zoom-out']")?.addEventListener("click", () => {
      cy.zoom({ level: Math.max(cy.zoom() / 1.2, cy.minZoom()), renderedPosition: center(container) });
    });
    shell.querySelector("[data-graph-action='fit']")?.addEventListener("click", () => cy.fit(undefined, 48));
    shell.querySelector("[data-graph-action='reset']")?.addEventListener("click", () => {
      cy.layout({ name: "breadthfirst", directed: true, spacingFactor: 1.35, padding: 48, animate: false }).run();
      cy.fit(undefined, 48);
    });

    shell.querySelectorAll("[data-highlight-path]").forEach((button) => {
