---
name: testing
description: Create and approve an optional GHERKIN.md test contract when the user explicitly requests one, or guide an implementation worker writing tests for approved unit, integration, or end-to-end scenarios; never invoke proactively otherwise.
---

# Testing

This skill has two roles: create and approve a human-reviewed test contract, and guide an implementation worker writing the approved tests. The active task's `BRIEF.md` is the behavioral authority, `PLAN.md` owns implementation and PR boundaries, and `GHERKIN.md` owns only the supplemental test behavior the user explicitly selected. An absent approved Gherkin section removes only Gherkin-specific requirements; it never removes tests already required by `PLAN.md`. The implementation workflow writes the resulting production and test code in the owning planned PR, and the implementation orchestrator independently validates it.
...
- **Worker route**: use only when an implementation worker receives one or more scenarios from an existing `GHERKIN.md` section that contains `Status: Approved`, `Approved: YYYY-MM-DD`, and `Audit: Passed YYYY-MM-DD`. Skip Steps 1-3 and continue directly to **Worker Test Routing**. If the assigned section is draft, lacks its approval or audit evidence, has an invalid execution mode, or contains no scenarios assigned to the worker's current `PLAN.md` PR, stop as blocked without editing tests.

## 1. Establish The Branch

Proceed only after the user explicitly invokes this workflow and selects exactly one test level. If the user invokes the workflow without a level or asks for guidance, classify the exercised boundary, recommend the smallest credible level that proves the requested behavior, explain why, and ask the user to select it. Never proceed with, infer approval for, or add a level merely because the implementation could benefit from it.
...
Classify by the boundary actually exercised, not the test runner or directory name. If the selected label conflicts with the requested boundary, explain the conflict, recommend the matching level, and ask the user to select again rather than silently reclassifying it. Require `BRIEF.md` and a published `PLAN.md` for every branch; stop and request the producing workflow when either is absent or when implementation requires an unresolved decision.

If the selected section is not approved, create or correct its draft, audit the current substantive revision, and stop for explicit approval. Preserve an approved section verbatim unless the user explicitly requests a substantive change; then remove its approval date, return it to `Status: Draft` and `Audit: Pending`, and audit the revised draft before requesting approval again. Stop on artifact contradictions instead of guessing.
...
Create or update `<active-task>/GHERKIN.md`. Preserve every other approved section verbatim. Give the selected branch its own `## Unit`, `## Integration`, or `## End-to-End` section followed, in order, by `Status: Draft`, its `Execution:` value, and `Audit: Pending`.

Write English Gherkin unless the repository establishes another specification language. Apply these rules:

- Give every scenario one tag such as `@UT-001`, `@IT-001`, or `@E2E-001` that is unique across the entire file, plus its level tag: `@unit`, `@integration`, or `@e2e`. Preserve IDs for scenarios that remain and never renumber an approved scenario; assign the next unused ID visible in the file when adding one.
...
After completing a new or substantively revised draft, invoke `test-auditor` for that current revision, supplying the active task path, full `BRIEF.md`, full `PLAN.md`, full current `GHERKIN.md`, and the full applicable branch reference. Require it to check whether the selected scope is complete and non-duplicative, including every applicable acceptance criterion and material invariant, representative success, rejection, boundary, and failure behavior, the selected test boundary, execution mode, observable outcomes, unique IDs, and exactly one correct owning PR per scenario. Correct valid findings and re-audit when the correction substantively changes scenarios or coverage. When the current revision is `READY`, replace `Audit: Pending` with `Audit: Passed YYYY-MM-DD`. Do not audit an unchanged revision again. The auditor never modifies `GHERKIN.md`.

Make final contract acceptance yourself and stop on a substantive artifact conflict or unresolved decision instead of guessing. End the contract-authoring run after presenting the audited draft and requesting explicit user approval. Test files remain unchanged. Approval applies only to the exact scenario text reviewed by the user; only the user can authorize changing the section from `Status: Draft` to `Status: Approved`. Any substantive scenario change returns the section to `Status: Draft` and `Audit: Pending`, requires a fresh audit of that revision, and requires user approval again. Approval-only continuations and worker implementation never invoke `test-auditor` for unchanged text.

This step is complete only when `GHERKIN.md` contains a complete, non-duplicative draft for the selected scope, every scenario has exactly one planned PR owner, the current substantive revision records a passed audit without changing approval status, and no plan, test, or production file has changed.

## 3. Confirm Approval

On a later continuation, incorporate requested Gherkin corrections, return the changed section to draft, audit the revised text, and stop again. Proceed only when the user explicitly approves the current selected section's exact scenario text and execution mode and its current revision records `Audit: Passed YYYY-MM-DD`. Change that section to `Status: Approved`, add `Approved: YYYY-MM-DD` immediately below it, and preserve its execution mode, audit evidence, scenarios, and traceability verbatim.
...
After routing, inspect the repository's existing tests, test configuration, fixtures, helpers, and production interfaces. Write the smallest tests that faithfully prove every assigned approved scenario, keep each scenario ID in its test name or adjacent parametrization ID, and follow all writing rules in the routed reference. Preserve the approved Gherkin text and its traceability. Do not create tests for unassigned scenarios or expand the selected level. The worker reports required test commands without running mapped Gherkin tests; the implementation orchestrator independently executes and validates them.

The contract route is complete when the approved supplemental contract is stable, audited, uniquely assigned to planned PRs, and ready for `/implement`; no plan, production, test, fixture, configuration, branch, or issue has changed on that route. The worker route is complete when every assigned approved scenario has a traceable test written according to its routed reference and the worker has reported the commands the orchestrator must validate.
