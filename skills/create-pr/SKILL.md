...
4. Record remote source and target SHAs. Inspect the complete remote merge-base diff (`git diff <target-ref>...<source-ref>`) and all included commits. Before drafting, make a private coverage ledger that maps every material behavior and every non-trivial commit to the relevant contract, implementation, observable impact, and representative source lines. Use relevant source and documentation at that exact state to write the Czech title and template below, not to perform code review. Exclude unrelated local changes and unpushed commits from the description. Before publication, recheck both SHAs; if either moved, refresh the affected diff, ledger, and description.
...
<Stejnou strukturu zopakuj pro každou další podstatnou změnu.>
...
- Scale description length to scope and complexity, but optimize for a thorough review aid rather than a short release note. As a default, a medium or large PR should use roughly 600-850 words total, counting prose, tables, and snippets. Treat 900 words for the entire description as a hard cap unless the user explicitly requests more. A small PR remains shorter. Do not achieve depth by repeating the same fact in multiple sections.
- `Overview` must cover every material behavior and impact, not just broad themes. Use 4-8 concise rows for a medium or large PR when the diff supports them; split independent contracts, lifecycle rules, race handling, migrations, and secondary user-visible changes instead of hiding them behind labels such as `Frontend` or `Backend`.
- `Detail změn` is the main body, not a summary. Give each coherent material change its own numbered subsection and normally use 4-7 subsections for a medium or large PR. In every subsection identify the concrete files, show the relevant code or contract, explain the runtime/data flow and important conditions or fallbacks, state what differs from the target branch, and explain why that shape is necessary when repository evidence establishes the reason.
- Include real, verbatim snippets from the remote source for the mechanisms a reviewer must understand. A medium or large code PR normally needs 4-7 snippets totaling about 30-70 lines; use 3-12 lines per snippet and include enough surrounding code to make branches, guards, payload shape, or state transitions understandable. Prefer public API/schema changes, migration operations, selection/fallback logic, concurrency or stale-result guards, and cross-layer handoffs. Use a compact request/response example only in addition to, not instead of, important implementation code. Omit snippets only for documentation-only changes or when no code fragment would add information.
- Never paste whole files, generated code, lockfiles, repetitive plumbing, or large test bodies. Do not include decorative snippets that merely restate a name or type. Every snippet must support an adjacent explanation of behavior and consequence.
- Before publication, cross-check the overview and details against the private coverage ledger, complete remote diff, and all included commits. Every non-trivial commit and material behavior must appear at least once, including secondary fixes that are not part of the PR title. Ensure relevant API/data contracts, state ownership, edge conditions, configuration, migrations, compatibility, and operational impacts are represented. If the draft exceeds the size budget, merge repetitive prose and remove low-value inventory; never remove a distinct behavior merely to shorten it.
- Describe only the remote diff, grouped by behavior, not a file inventory. Explain changes relative to the actual PR target, not always the integration branch.
...
- Final depth check: a reviewer should be able to answer from the description alone who owns changed state, what each changed boundary accepts or returns, how the main happy path runs, which guards/fallbacks prevent invalid or stale behavior, what data or deployment action is required, and which material commit changed behavior outside the headline. If the description only names files and outcomes, deepen it with source-backed code and mechanics before publishing.
