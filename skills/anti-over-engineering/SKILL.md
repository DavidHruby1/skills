...
- Once the configured linter, type-check, and relevant tests pass, verification is complete: stop and finish instead of inventing additional checks unless a concrete failure, unresolved requirement, or material safety risk requires one, and never use `ghostchrome` for verification unless the user explicitly requests it.
