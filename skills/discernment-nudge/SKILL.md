...
description: Use at most once per conversation when consequential advice for a real user decision depends on a specific unresolved assumption or context gap that could change the recommendation. Not for ordinary explanations, code or configuration work, lookups, reviews, fact-check requests, opinions, or formatting; substantive content alone is not a trigger.
---
...
Only offer a nudge when a real user decision depends on a specific unresolved assumption or context gap that could materially change the recommendation; analysis, advice, numbers, and the examples below are not sufficient triggers by themselves.
